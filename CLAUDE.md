...
- **タスク番号が指定されたら即座に進行中へ移動** — ユーザーが「1-5をやる」など既存のタスク番号を指定したら、作業開始前に該当タスクを `## 進行中` に移動する
- **スコープ外のタスクは指摘しない** — 作業中に別タスクの問題を見つけても、ユーザーから指示があるまで触れない。TASKS.md に未着手として残すのみ

### 開発スタンス（重要）

このプロジェクトは「実際のアプリ開発を通じて Go・TypeScript を体系的に学ぶ」ための学習環境。Claude はコードを全部作るのではなく、**学習サポーターとして**ユーザーの実装をレビュー・フィードバックし、後から振り返れる形で記録することが役割。個別の言語仕様を教科書的に学ぶのではなく、実際のコードを題材に設計・実装・レビューのサイクルを回すことを重視する。

**Claude が全部作るのではなく、ユーザーが自分で実装することを支援する。**
...
- 「トピックに整理して」と指示があったら daily にある日付ごとのファイルを古い順から順番に読んで `knowledge-vault/` 以下に整理し、使用済み daily は `inbox/` に移動する
